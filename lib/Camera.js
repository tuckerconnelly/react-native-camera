...
if(_this.props.onBarCodeRead)_this.props.onBarCodeRead(data);
};_this.state={isAuthorized:false,isRecording:false};return _this;}_createClass(Camera,[{key:'componentWillMount',value:function componentWillMount(){var _convertNativeProps,captureMode,hasVideoAndAudio,check,isAuthorized;return regeneratorRuntime.async(function componentWillMount$(_context){while(1){switch(_context.prev=_context.next){case 0:this.cameraBarCodeReadListener=_reactNative.NativeAppEventEmitter.addListener('CameraBarCodeRead',this._onBarCodeRead);_convertNativeProps=convertNativeProps({captureMode:this.props.captureMode});captureMode=_convertNativeProps.captureMode;hasVideoAndAudio=this.props.captureAudio&&captureMode===Camera.constants.CaptureMode.video;check=hasVideoAndAudio?Camera.checkDeviceAuthorizationStatus:Camera.checkVideoAuthorizationStatus;if(!check){_context.next=10;break;}_context.next=8;return regeneratorRuntime.awrap(check());case 8:isAuthorized=_context.sent;this.setState({isAuthorized:isAuthorized});case 10:case'end':return _context.stop();}}},null,this);}},{key:'componentWillUnmount',value:function componentWillUnmount(){this.cameraBarCodeReadListener.remove();if(this.state.isRecording){this.stopCapture();}}},{key:'render',value:function render(){var nativeProps=convertNativeProps(this.props);return _react2.default.createElement(RCTCamera,_extends({ref:CAMERA_REF},nativeProps));}},{key:'capture',value:function capture(
